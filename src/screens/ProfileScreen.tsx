    ...
    <div className="flex flex-col gap-6 pb-10">
      ...
      <div className="px-6 pb-4">
        <div className="rounded-2xl border border-white/5 bg-white/[0.03] px-5 py-4">
          <p className="text-[9px] font-black uppercase tracking-widest text-white/20 mb-3">Phiên bản ứng dụng</p>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[10px] text-white/40">Đang dùng</p>
              <p className="text-sm font-black text-white font-mono">{currentVersion}</p>
            </div>
            <div className="text-right">
              <p className="text-[10px] text-white/40">Mới nhất</p>
              <p className={`text-sm font-black font-mono ${checking ? 'text-white/30' : isUpToDate ? 'text-green-400' : 'text-yellow-400'}`}>
                {checking ? '...' : latestVersion || '?'}
              </p>
            </div>
            <div className={`rounded-xl px-3 py-2 text-[9px] font-black uppercase ${isUpToDate ? 'bg-green-500/10 text-green-400' : 'bg-yellow-500/10 text-yellow-400'}`}>
              {checking ? '🔄' : isUpToDate ? '✓ Mới nhất' : '↑ Cần cập nhật'}
            </div>
          </div>
        </div>
      </div>
